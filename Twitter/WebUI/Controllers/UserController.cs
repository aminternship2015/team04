...
namespace WebUI.Controllers
{
    public class UserController : Controller
    {
        ...
        {
            var currentUser = (UserViewModel)HttpContext.Session["CurrentUser"];
            //var users = userService.GetAll(currentUser.Id).Where
            //    (x => x.First_name.ToLower() == name.ToLower() || x.Last_name.ToLower() == name.ToLower() || name.ToLower() == (x.First_name.ToLower() + " " + x.Last_name.ToLower())).ToList();
            //return View(users);

            var sentences = userService.GetAll(currentUser.Id).ToList();
            ...
            foreach (var s in sentences)
            {
                ...
                if (Regex.IsMatch(s.First_name, sPattern, System.Text.RegularExpressions.RegexOptions.IgnoreCase) ||
                    Regex.IsMatch(s.Last_name, sPattern, System.Text.RegularExpressions.RegexOptions.IgnoreCase) ||
                    Regex.IsMatch(s.First_name + " " + s.Last_name, sPattern, System.Text.RegularExpressions.RegexOptions.IgnoreCase))
                {
                    newlist.Add(s);
                }
            }
